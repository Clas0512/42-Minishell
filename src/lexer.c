...
void    init_struct(t_lex *info, char *str)
{
	...
	info->s_q = 0;
	info->d_q = 0;
	info->ln = malloc(sizeof(t_line) * 2);
	info->ln->a = 0;
	info->ln->b = 0;
}
...
char **lexer(char *str)
{
	...
	info.ln->line = malloc(sizeof(char *) * (info.word_count + 1));
	letter_manager(&info, str, info.ln->line);
	set_line(&info, str, info.ln->line);
	i = 0;
	while (i < info.word_count)
	{
		while (info.ln->line[i] != 0)
			printf("%s\n", info.ln->line[i]);
		   i++;
	}
	return (info.ln->line);
